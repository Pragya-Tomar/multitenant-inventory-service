...
public class DealerService {
    ...
    public DealerResponse createDealer(CreateDealerRequest req) {
        ...
        Dealer dealer = Dealer.builder()
                ...
                .email(req.email()).subscriptionType(req.subscriptionType())
                .build();
    ...
    public DealerResponse updateDealer(UUID id, UpdateDealerRequest req) {
        ...
        if (StringUtils.hasText(req.name())) dealer.setName(req.name());
        if (StringUtils.hasText(req.email())) dealer.setEmail(req.email());
        if (req.subscriptionType() != null) dealer.setSubscriptionType(req.subscriptionType());
        return DealerResponse.from(dealerRepository.save(dealer));
    }

    @Transactional
    public void deleteDealer(UUID id) {
        dealerRepository.delete(findTenantScoped(id));
    }
